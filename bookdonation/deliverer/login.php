    ...
    $success="";
                if($_POST){
                     ...
                     if($error!=""){
                        ...
                        $check="SELECT `deliverer_id`, `email`, `password` FROM `deliverers` WHERE  email = '".$email."' AND password='".$password1."'";
                        //checking the result of query
                        $result=$conn->query($check);
                            if ($result->num_rows==1){
                            
                                     while($row = mysqli_fetch_array($result))
                                      $_SESSION['login_user'] = $row['deliverer_id'];
...
<html>
      <head>
         ...
         <script src="js/delieverr.js" type="text/javascript"></script>
      </head>
      <body >
          <nav class="navbar fixed-top mb-3 pb-2" style="background-color:#047069;">
            <a class="mark" style="text-align:center;background-color:#047069; color:white;" href="home.php">BOOK+</a>
          </nav>
           <div class="container ">
